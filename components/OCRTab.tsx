...
import ExpoOCRModule from '../modules/expo-ocr/src/ExpoOCRModule';
...
const startOCRScanning = async (serverURL: string, transactionID: string, documentType: string, documentSide: string) => {
  if (!ExpoOCRModule) throw new Error('OCRModule not available');
  return await ExpoOCRModule.startOCRScanning(serverURL, transactionID, documentType, documentSide);
};

const configureUISettings = async (uiConfig: any) => {
  if (!ExpoOCRModule) throw new Error('OCRModule not available');
  if (!ExpoOCRModule.configureUISettings) {
...
const performOCR = async (serverURL: string, transactionID: string, frontSideImage: string, backSideImage: string, documentType: string) => {
  if (!ExpoOCRModule) throw new Error('OCRModule not available');
  return await ExpoOCRModule.performOCR(serverURL, transactionID, frontSideImage, backSideImage, documentType);
};

const performDocumentLiveness = async (serverURL: string, transactionID: string, frontSideImage: string, backSideImage: string) => {
  if (!ExpoOCRModule) throw new Error('OCRModule not available');
  return await ExpoOCRModule.performDocumentLiveness(serverURL, transactionID, frontSideImage, backSideImage);
};
...
const performOCRAndDocumentLiveness = async (serverURL: string, transactionID: string, frontSideImage: string, backSideImage: string, documentType: string) => {
  if (!ExpoOCRModule) throw new Error('OCRModule not available');
  return await ExpoOCRModule.performOCRAndDocumentLiveness(serverURL, transactionID, frontSideImage, backSideImage, documentType);
};

const startHologramCamera = async (serverURL: string, transactionID: string) => {
  if (!ExpoOCRModule) throw new Error('OCRModule not available');
  return await ExpoOCRModule.startHologramCamera(serverURL, transactionID);
};
...
const OCRTab: React.FC = () => {
  ...
  useEffect(() => {
    const ocrCompleteListener = ExpoOCRModule.addListener('onOCRComplete', (result) => {
    ...
    const ocrErrorListener = ExpoOCRModule.addListener('onOCRError', (error) => {
      console.error('OCRTab - ❌ OCR Error:', error);
      setIsOcrLoading(false);
      Alert.alert('OCR Error', error.message || 'Unknown error');
    });

    const hologramCompleteListener = ExpoOCRModule.addListener('onHologramComplete', async (result) => {
    ...
    const hologramVideoRecordedListener = ExpoOCRModule.addListener('onHologramVideoRecorded', (event) => {
    ...
    const hologramErrorListener = ExpoOCRModule.addListener('onHologramError', (error) => {
  ...
  async function requestPermissions(): Promise<boolean> {
    try {
      ...
      // For iOS, we might need additional permissions
      if (Platform.OS === 'ios') {
        // Request notification permissions if needed for background processing
        if (Device.isDevice) {
          ...
          if (finalStatus !== 'granted') {
            console.warn('OCRTab - Notification permission not granted, but continuing...');
          }
        }
      }
  ...
  const handleApplyUICustomization = async (config: any) => {
    try {
      ...
      console.log('OCRTab - ExpoOCRModule available:', !!ExpoOCRModule);
      console.log('OCRTab - configureUISettings available:', !!(ExpoOCRModule && ExpoOCRModule.configureUISettings));
