...
import ExpoNfcModule from '../modules/expo-nfc';

const isNFCAvailable = async () => {
  if (!ExpoNfcModule) throw new Error('ExpoNfcModule not available');
...
const getNFCStatus = async () => {
  try {
    ...
    console.error('ExpoNfcModule - getNFCStatus error:', error);
    return {
      isAvailable: false,
      isEnabled: false,
...
const NFCTab: React.FC<NFCTabProps> = ({ mrzData }) => {
  ...
  async function requestPermissions(): Promise<boolean> {
    if (Platform.OS === 'android') {
      try {
        console.log('NFCTab - Checking Android permissions...');
        
        // NFC permissions are handled in AndroidManifest.xml
        // READ_PHONE_STATE is also declared in manifest and handled by Udentify SDK
        // No runtime permission request needed for Expo
        
        console.log('NFCTab - Permissions handled by manifest');
        return true;
      } catch (error) {
        console.error('NFCTab - Permission check error:', error);
        return false;
      }
    }
    return true;
    ...
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.buttonContainer}>
            <Button 
              key={`start-reading-${isReading}-${isAvailable}-${isEnabled}`}
              color={'green'} 
          ...
          <View style={styles.buttonContainer}>
            <Button 
              key={`nfc-location-${isReading}-${isAvailable}-${isEnabled}`}
              color={'blue'} 
