...
import * as ExpoLiveness from '../modules/expo-liveness';
import * as ExpoOCR from '../modules/expo-ocr';

const checkPermissions = ExpoLiveness.checkPermissions;
const requestPermissions = ExpoLiveness.requestPermissions;
const startFaceRecognitionRegistration = ExpoLiveness.startFaceRecognitionRegistration;
const startFaceRecognitionAuthentication = ExpoLiveness.startFaceRecognitionAuthentication;
const startActiveLiveness = ExpoLiveness.startActiveLiveness;
const startHybridLiveness = ExpoLiveness.startHybridLiveness;
const startSelfieCapture = ExpoLiveness.startSelfieCapture;
const performFaceRecognitionWithSelfie = ExpoLiveness.performFaceRecognitionWithSelfie;
const cancelFaceRecognition = ExpoLiveness.cancelFaceRecognition;
const isFaceRecognitionInProgress = ExpoLiveness.isFaceRecognitionInProgress;

const startOCRScanning = ExpoOCR.startOCRScanning;
const performOCR = ExpoOCR.performOCR;
...
const LivenessTab: React.FC = () => {
  ...
  useEffect(() => {
    const faceRecognitionResultListener = ExpoLiveness.addFaceIDCompleteListener((result) => {
    ...
    const faceRecognitionFailureListener = ExpoLiveness.addFaceIDErrorListener((error) => {
      console.log('\n🚨 ========== LIVENESS FAILURE ==========');
    ...
    const photoTakenListener = ExpoLiveness.addPhotoTakenListener(() => {
      console.log('\n📸 ========== PHOTO TAKEN ==========');
    ...
    const selfieTakenListener = ExpoLiveness.addSelfieTakenListener((eventData) => {
    ...
    const activeLivenessResultListener = ExpoLiveness.addActiveLivenessResultListener((result) => {
    ...
    const activeLivenessFailureListener = ExpoLiveness.addActiveLivenessFailureListener((error) => {
    ...
    const willDismissListener = ExpoLiveness.addWillDismissListener(() => {
      console.log('\n🚪 ========== WILL DISMISS ==========');
    ...
    const didDismissListener = ExpoLiveness.addDidDismissListener(() => {
      console.log('\n🚪 ========== DID DISMISS ==========');
    ...
    const backButtonListener = ExpoLiveness.addBackButtonPressedListener(() => {
    ...
    // OCR Event Listeners (for OCR dependency) - not yet implemented in Expo OCR module
    // const ocrCompleteListener = ...;
    // const ocrErrorListener = ...;
  ...
  }, [selfieOperation, selectedUserID, currentTransactionID]);
  ...
  // Monitor OCR completion state changes
  useEffect(() => {
    console.log('LivenessTab - 🔍 OCR Completed State Changed:', ocrCompleted);
    console.log('LivenessTab - 🎯 Buttons should now be:', ocrCompleted ? 'ENABLED (colored)' : 'DISABLED (gray)');
  }, [ocrCompleted]);
  ...
  async function configureUIAndLocalization() {
    ...
    try {
      ...
      if (ExpoLiveness.configureUISettings) {
        ...
        await ExpoLiveness.configureUISettings(uiSettings);
    ...
    try {
      if (ExpoLiveness.setLocalization) {
        ...
        await ExpoLiveness.setLocalization('en', customStrings);
  ...
  async function handleRequestPermissions() {
    try {
      ...
      const permissions = await requestPermissions() as unknown as FaceRecognitionPermissionStatus;
      setPermissionStatus(permissions);
  ...
  async function performOCRFirst(): Promise<boolean> {
    try {
      ...
      const transactionID = await udentifyApiService.startTransaction([
        ...
        'FACE_REGISTRATION' as any,
        'FACE_AUTHENTICATION' as any,
        'FACE_LIVENESS' as any,
        'ACTIVE_LIVENESS' as any
      ]);
      ...
      if (success) {
        ...
        console.log('LivenessTab - 🔄 Setting OCR completed state to TRUE...');
        
        setOcrResult(ocrResult);
        setOcrCompleted(true);
        setIsLoading(false);
        
        console.log('LivenessTab - ✅ OCR state updated - buttons should now be enabled');
  ...
  const handleUIConfigApply = async (config: any) => {
    try {
      ...
      if (ExpoLiveness.configureUISettings) {
        const result = await ExpoLiveness.configureUISettings(config);
    ...
    <ScrollView style={styles.container}>
      <View style={styles.content}>
        ...
        <View style={styles.section}>
          ...
          {/* Debug State Display */}
          <View style={[styles.infoContainer, { backgroundColor: ocrCompleted ? '#e8f5e8' : '#fff3e0' }]}>
            <Text style={[styles.infoText, { color: ocrCompleted ? '#2e7d32' : '#f57c00' }]}>
              🔍 DEBUG: ocrCompleted = {ocrCompleted ? 'TRUE ✅' : 'FALSE ❌'}
            </Text>
            <Text style={[styles.infoText, { fontSize: 12, marginTop: 4 }]}>
              Expected button states: Registration = {ocrCompleted ? 'GREEN' : 'GRAY'}
            </Text>
          </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.buttonContainer}>
            <Button 
              key={`face-auth-${ocrCompleted}-${checkUserRegistrationStatus(selectedUserID)}`}
              color={'#2196F3'} 
          ...
          <View style={styles.buttonContainer}>
            <Button 
              key={`active-liveness-${ocrCompleted}-${selectedOperation}-${checkUserRegistrationStatus(selectedUserID)}`}
              color={'#9C27B0'} 
          ...
          <View style={styles.buttonContainer}>
            <Button 
              key={`hybrid-liveness-${ocrCompleted}-${selectedOperation}-${checkUserRegistrationStatus(selectedUserID)}`}
              color={'#FF9800'} 
        ...
        <View style={styles.section}>
          ...
          <View style={styles.buttonContainer}>
            <Button 
              key={`capture-selfie-${ocrCompleted}-${selfieOperation}-${checkUserRegistrationStatus(selectedUserID)}`}
              color={'#FF6B35'} 
        ...
        {displaySelfie && (
          <View style={styles.section}>
            ...
            <View style={[styles.resultContainer, { alignItems: 'center' }]}>
              ...
              <TouchableOpacity 
                style={{ backgroundColor: '#dc3545', marginTop: 10, padding: 12, borderRadius: 8, alignItems: 'center' }}
                onPress={() => {
                  setDisplaySelfie(null);
                  setCapturedSelfieImage(null);
                }}
              >
                <Text style={{ color: 'white', fontSize: 16, fontWeight: '600' }}>🗑️ Clear Image</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
